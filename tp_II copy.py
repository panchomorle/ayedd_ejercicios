...
while(minuto <= fin_de_la_jornada): 
    ...
    for caja in range(cantidad_de_cajas):
        for persona in range(len(cajas[caja])):
            ...
            #si no es la primera persona en la cola ni salió de la caja recientemente (fuera de rango)
            if persona !=0 and not out_of_range: 
                p = cajas[caja][persona] #defino la persona
                p["tiempo_esperando_en_cola"] += 1
                if p["tiempo_esperando_en_cola"] > p["grado_de_paciencia"]:
    ...
    for caja in range(cantidad_de_cajas):
        if len(cajas[caja]) > 0: #compruebo si hay personas en la caja
            p = cajas[caja][0]
            p["tiempo_esperando_en_caja"] += 1 #0 es la persona 0 en la cola, o sea el que está siendo atendido.
            if p["tiempo_esperando_en_caja"] >= p["tiempo_que_tarda_la_caja"]:
                cantidad_personas_atendidas +=1
                minutos_demorados_caja += p["tiempo_que_tarda_la_caja"]
                cajas[caja].pop(0)
